...
- **`status` no longer presents expired quota snapshots as live when the footer is disabled.** With `showUsage: false`, background polling correctly stopped, but the all-account status table continued rendering persisted snapshots without refreshing them. Once a reset timestamp passed this produced contradictions such as `已用尽 · 0m 后重置` beside `全部账号当前可用`. Every `/status` invocation now bypasses the cache and requests current quota once for every authenticated rotation account; explicit `limits [refresh]` queries also work independently of the footer setting. Automatic polling remains off, so disabling the footer stays network-quiet between commands.
