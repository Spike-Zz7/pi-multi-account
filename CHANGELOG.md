...
- **Routine conditions no longer overuse warning notifications.** Duplicate account slots are skipped safely without nagging on every session start and remain visible in `/status`; successful user-requested `next`, `best`, and `switch` operations now produce only their normal confirmation instead of an extra fake failover warning; identical persistent internal faults notify once while remaining fully recorded in the debug log; and providers without quota telemetry are shown as neutral status information rather than a fault. `/status full` retains the underlying diagnostics.

- **Model rotation no longer raises a false `settings.json` contract warning.** Pi's `defaultProvider` and `defaultModel` are optional startup preferences, not live-session state: Pi 0.84 changes them only when the user explicitly saves a default, while ordinary model switches and the extension API's `pi.setModel()` deliberately leave them unchanged. The extension no longer compares those defaults with the active rotation model or claims that a bare child automatically inherits the current account. Child compatibility remains reported per slot, and the saved startup route is still checked on its own terms.

- **`status` no longer presents expired quota snapshots as live when the footer is disabled.** With `showUsage: false`, background polling correctly stopped, but the all-account status table continued rendering persisted snapshots without refreshing them. Once a reset timestamp passed this produced contradictions such as `已用尽 · 0m 后重置` beside `全部账号当前可用`. `/status` now refreshes every authenticated rotation account whose normal cache TTL has expired (Codex: five minutes by default; Anthropic: at least ten minutes), while repeated commands inside that window reuse the cache; explicit `limits [refresh]` queries work independently of the footer setting.

- **Different members of one ChatGPT Team workspace no longer collide as duplicate Codex accounts.** Codex tokens carry both a workspace-level `chatgpt_account_id` and a user-level `chatgpt_user_id`, but duplicate detection used only the workspace id. A Team owner and an invited member therefore produced the same identity, the member login was rejected, and an already-saved member slot was omitted from rotation and shared the owner's cooldown. Codex identity now combines workspace and user ids when the JWT exposes both, while retaining the stored workspace id as a compatibility fallback for credentials without a decodable user claim. The same identity is used by login rejection, discovery deduplication, and cooldown propagation.
