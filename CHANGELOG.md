...
- **`/multi-account status` is concise by default.** The normal view now keeps the current account, limits, rotation, recovery, child-process compatibility, actionable warnings, and four common actions in a short scan-friendly block. Healthy internal diagnostics, the full slot registry, watchdog/breaker state, paths, and the exhaustive command list move to `/multi-account status full` (aliases: `verbose`, `details`). Problems such as cooldowns, invalid credentials, duplicates, unconfigured models, queued work, and file-contract drift still appear automatically in the compact view.
