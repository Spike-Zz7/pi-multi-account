...
- **`/multi-account status` is concise and account-oriented by default.** The normal view now uses Pi theme colors and identifies a subscription as `email: Team/Plus/...` instead of exposing internal slot names such as `Codex A2` or `openai-codex-account-2`. It keeps the model, limits, human-readable rotation, recovery, child-process compatibility, actionable warnings, and common actions in a short scan-friendly block. Healthy internal diagnostics, exact provider ids, the full slot registry, watchdog/breaker state, paths, and the exhaustive command list move to `/multi-account status full` (aliases: `verbose`, `details`). Problems such as cooldowns, invalid credentials, duplicates, unconfigured models, queued work, and file-contract drift still appear automatically in the compact view.
