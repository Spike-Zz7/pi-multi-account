...
- **Deduplicates provably identical accounts** so duplicate Codex workspace/member identities and identical credentials do not consume multiple rotation slots or get separate cooldowns. Different members of one ChatGPT Team workspace remain separate rotation accounts. New provable duplicate logins are rejected before the redundant slot is saved.
...
- **Duplicate identities** share one rotation position and one cooldown, and status/startup identifies the redundant slot. Codex/ChatGPT JWTs are matched by workspace plus user identity, so two members of the same Team workspace do not collide; stored `accountId` remains the fallback when no user identity can be decoded. Identical API keys or literal identical tokens are also matched. Separate Anthropic OAuth logins cannot be proven identical because Anthropic's stored credential exposes no stable account identifier.
